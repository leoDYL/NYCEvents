...
### Top Event Categories, Organizers, Locations and Durations
...
nyc_event_listing_events %>%
  mutate(Duration=difftime(strptime(paste(Dt, ET), "%Y-%m-%d %H:%M"), strptime(paste(Dt, ST), "%Y-%m-%d %H:%M"))/3600) %>%
  filter(Duration > 0) %>%
  ggplot(aes(Duration)) +
...
### Top Events Categories for Inidividual Organizers
```{r, fig.width=14, fig.height=14}
nyc_events_left_joined %>%
  drop_na(Org, Cat) %>%
  subset(Org %in% top_organizers$Org) %>%
  subset(Cat %in% top_categories$Cat) %>%
  group_by(Org, Cat) %>%
  summarize(Count=n_distinct(EvtId)) %>%
  ggplot(aes(Cat, Count)) +
  geom_col() +
  facet_wrap(.~Org, ncol = 4) +
  ggtitle("Barchart for Event Categories by Organizers") +
  labs(x = "Event Category", y = "Count of Events") +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
  
```

### Top Events Locations for Inidividual Organizers
```{r, fig.width=14, fig.height=14}
nyc_events_left_joined %>%
  drop_na(Org, LcNm) %>%
  subset(Org %in% top_organizers$Org) %>%
  subset(LcNm %in% top_locations$LcNm) %>%
  group_by(Org, LcNm) %>%
  summarize(Count=n_distinct(EvtId)) %>%
  ggplot(aes(LcNm, Count)) +
  geom_col() +
  facet_wrap(.~Org, ncol = 4) +
  ggtitle("Barchart for Event Locations by Organizers") +
  labs(x = "Event Location", y = "Count of Events") +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
...
### Top Organizers' Event Categories and Locations Preferences
...
nyc_events_left_joined[c("EvtId", "Org", "Cat")] %>%
  filter(Org %in% top_organizers$Org) %>%
  filter(Cat %in% top_categories$Cat) %>%
  group_by(Org, Cat) %>%
  summarize(UniqueEvts = n_distinct(EvtId))


```


<!-- TODO Top Park's Most Frequent Event Categories??? -->

<!-- TODO Events with multiple organizers / the most "collaborative" organizers -->
...
nyc_events_left_joined %>%
  drop_na(EvtId, LcNm) %>%
  group_by(LcNm) %>%
  summarize(num_events=n_distinct(EvtId)) %>%
  arrange(desc(num_events)) %>%
  top_n(20) %>%
  ggplot(aes(x = num_events, y = fct_reorder(LcNm, num_events))) +
  geom_point(color = "blue") +
...
nyc_events_joined %>%
  ...
  #filter(n() > 100) %>%
  summarize(free_ratio=sum(CsFr)/n()) %>%
...
nyc_events_timeseries_weekly <- nyc_event_listing_events %>%
  filter(Dt < as.Date("2019-01-01")) %>%
  mutate(Month = month(Dt, label=TRUE, abbr=TRUE), Wday = wday(Dt, label = TRUE, abbr = TRUE)) %>%
  group_by(Month, Wday) %>%
  summarize(Count = n())
