...
nyc_event_listing_events %>%
  mutate(Duration=sprintf("%.2f",difftime(strptime(paste(Dt, ET), "%Y-%m-%d %H:%M"), strptime(paste(Dt, ST), "%Y-%m-%d %H:%M"))/3600)) %>%
  filter(Duration > 0) %>%
  group_by(Duration) %>%
  summarize(count=n()) %>%
  top_n(30) %>%
  ggplot(aes(x=fct_reorder(Duration,as.numeric(Duration)),y=count)) +
