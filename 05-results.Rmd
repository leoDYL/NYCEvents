...
free_mosaic$CsFr = as.factor(ifelse(free_mosaic$CsFr == 0, "Not Free", "Free"))
free_mosaic$Bo = as.factor(free_mosaic$Bo)
free_mosaic$Duration = fct_relevel(free_mosaic$Duration, levels = c("< 1", "1-3", "3-5", ">=5"))
free_mosaic$TimeofDay = fct_relevel(free_mosaic$TimeofDay, levels = c("Morning", "Afternoon", "Evening", "Night"))

mosaic_select <- free_mosaic %>%
  select(CsFr, Bo, Duration, TimeofDay)

pairs(table(mosaic_select), highlighting=2)
```
...
```{r, fig.width=9}
nyc_crimes_timeseries_weekly <- nyc_parks_crime %>%
  drop_na(Boro, Qtr) %>%
  group_by(Boro, Qtr) %>%
  summarize(Mdr = sum(Mdr), 
            Rape = sum(Rape), 
...
ggplot(nyc_crimes_timeseries_weekly, aes(Qtr, Count, group=Boro, color=Boro)) +
  geom_line(size = 0.5) +
