...
TODO Top Organizer's Favorite Locations

TODO Top Organizer's Favorite Event Categories
...
```{r}
nyc_events_joined %>%
  drop_na(EvtId, LcNm, Cat) %>%
  group_by(LcNm) %>%
  summarise(num_cats=length(unique(Cat))) %>%
  arrange(desc(num_cats)) %>%
  top_n(20) %>%
  ggplot(aes(x = num_cats, y = fct_reorder(LcNm, num_cats))) +
  geom_point(color = "blue") +
